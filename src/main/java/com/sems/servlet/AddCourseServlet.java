/**
 *
 * @author maisarahabjalil
 *
 *  doGet   -   grabs all the available courses
 *  doPost  -   creates a new record in enrollments table
 *
 *  adds activity to activity_logs
 *
 */
package com.sems.servlet;

import com.sems.dao.CourseDAO;
import com.sems.dao.EnrollmentDAO;
import com.sems.dao.StudentDAO;
import com.sems.model.Course;
import com.sems.model.Student;
import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;
import jakarta.servlet.http.HttpSession;
import java.io.IOException;
import java.util.HashSet;
import java.util.List;
import java.util.Set;
import com.sems.dao.ActivityLogDAO;

@WebServlet("/student/AddCourseServlet")
public class AddCourseServlet extends HttpServlet {

    private CourseDAO courseDAO = new CourseDAO();
    private EnrollmentDAO enrollmentDAO = new EnrollmentDAO();
    private StudentDAO studentDAO = new StudentDAO();
    private ActivityLogDAO logDAO = new ActivityLogDAO();

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        HttpSession session = request.getSession();
        Integer userId = (Integer) session.getAttribute("userId");

        if (userId == null) {
            response.sendRedirect(request.getContextPath() + "/login.jsp");
            return;
        }

        try {
            // 1. Get the Student Profile (for the sidebar GPA and Name)
            Student student = studentDAO.getStudentByUserId(userId);

            if (student != null) {
                // 2. Get available courses (filtering out courses the student already finished)
                List<Course> allCourses = courseDAO.getAvailableCoursesForStudent(student.getStudentId());

                // 3. Get IDs of courses the student is CURRENTLY enrolled in (to show the "check-double" icon)
                List<Course> enrolled = enrollmentDAO.getEnrolledCourseDetails(student.getStudentId());
                Set<Integer> enrolledIds = new HashSet<>();
                for (Course c : enrolled) {
                    enrolledIds.add(c.getCourseId());
                }

                // 4. Set attributes for the JSP
                request.setAttribute("student", student);
                request.setAttribute("allCourses", allCourses);
                request.setAttribute("enrolledIds", enrolledIds);

                // 5. Forward to the JSP
                request.getRequestDispatcher("/student/addcourse.jsp").forward(request, response);
            } else {
                response.sendRedirect(request.getContextPath() + "/login.jsp?error=noProfile");
            }
        } catch (Exception e) {
            e.printStackTrace();
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
        }
    }

    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        HttpSession session = request.getSession();
        Integer userId = (Integer) session.getAttribute("userId");
        Integer studentId = (Integer) session.getAttribute("studentId");

        if (userId == null || studentId == null) {
            response.sendRedirect(request.getContextPath() + "/login.jsp");
            return;
        }

        try {
            int courseId = Integer.parseInt(request.getParameter("courseId"));
            String courseName = request.getParameter("courseName"); // Pass this from your JSP form

            // Perform the actual enrollment
            boolean success = enrollmentDAO.enrollStudent(studentId, courseId);

            if (success) {
                // TRIGGER THE LOG: Record the activity!
                logDAO.recordLog(
                        userId,
                        courseId,
                        "ENROLL",
                        "Successfully enrolled in " + courseName
                );

                response.sendRedirect(request.getContextPath() + "/student/MyCourseServlet?success=enrolled");
            } else {
                response.sendRedirect(request.getContextPath() + "/student/AddCourseServlet?error=failed");
            }
        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect(request.getContextPath() + "/student/AddCourseServlet?error=exception");
        }
    }
}
